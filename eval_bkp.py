...
def multithread_eval(image_list, lpips_net, wh_bg):
    executor = concurrent.futures.ThreadPoolExecutor(max_workers=None)

    cum_psnr = [None] * image_list[0].shape[0]
    cum_ssim = [None] * image_list[0].shape[0]
    cum_lpips = [None] * image_list[0].shape[0]
    count = 0

    def eval_image(gt, pred, index, wh_bg):
        try:
            # print(view_pair[0].shape)
            # gt = view_pair[0].cpu().numpy()
            # pred = view_pair[1].cpu().numpy()
            gt = gt.cpu().numpy()
            pred = pred.cpu().numpy()
            ...
            pred = pred.astype(np.float32)
            pred /= 255.
            lpips = float(lpips_loss(pred, gt, lpips_net))
            print(lpips)

            cum_psnr[index] = psnr
            cum_ssim[index] = ssim
            cum_lpips[index] = lpips
    ...
    tasks = []
    for index in range(image_list[0].shape[0]):

        tasks.append(executor.submit(eval_image, image_list[0][index], image_list[1][index], index, wh_bg))
    executor.shutdown()
    for _ in concurrent.futures.as_completed(tasks):
        index, status = _.result()
        if status == False:
            index, status = eval_image(image_list[0][index], image_list[1][index], index, wh_bg)
        count += 1
    
    # print(cum_psnr)
    return (np.sum(cum_psnr), np.sum(cum_ssim), np.sum(cum_lpips), count)



def eval_set(model_path, device = torch.device('cuda'), wh_bg=False):

    # Each experiment is expected to be in the form: model/checkpoint/gt/camXX/%05d.png
    # gt_paths = glob.glob(model_path+'/gt/*/*.png')
    dataset = evalDataset(model_path)
    dataloader = DataLoader(dataset, num_workers = 12, batch_size = 12)
    ...
    for view_pair in tqdm(iter(dataloader), desc = f"Model: {model_path.split('/')[-3]} Split: {model_path.split('/')[-2]} Checkpoint: {model_path.split('/')[-1]}", position = 1):
        ...
        # avg_ssim += ssim
        # avg_lpips += lpips
        results = multithread_eval(view_pair, lpips_net, wh_bg)
